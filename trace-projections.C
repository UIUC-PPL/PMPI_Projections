...
#define CMK_PROJECTIONS_USE_ZLIB 0
...
void traceThreadListener_resume(struct CthThreadListener *l) 
{
  ...
  a->event=-1;
  a->srcPe=myPe; /* potential lie to migrated threads */
  a->ml=0;
}
...
LogPool::LogPool(char *pgm, int _myPe) {
...
void TraceProjections::traceBegin(void)
{
  ...
  _logPool->add(BEGIN_TRACE, 0, 0, TraceTimer(), curevent++, myPe);
}

void TraceProjections::traceEnd(void)
{
  _logPool->add(END_TRACE, 0, 0, TraceTimer(), curevent++, myPe);
}

void TraceProjections::userEvent(int e)
{
  if (!computationStarted) return;
  _logPool->add(USER_EVENT, e, 0, TraceTimer(),curevent++,myPe);
}
...
void TraceProjections::userBracketEvent(int e, double bt, double et)
{
  ...
  _logPool->add(USER_EVENT_PAIR, e, 0, TraceTimer(bt), curevent, myPe);
  _logPool->add(USER_EVENT_PAIR, e, 0, TraceTimer(et), curevent++, myPe);
}
					 ...
					 int *pelist)
{
  ...
  if (e==0) {
    ...
    _logPool->addCreationMulticast(ForChareMsg, ep, curTime, curevent++,
				   myPe, 0, 0, 0.0, num, pelist);
  } else {
    int type=e->getMsgtype();
    e->setEvent(curevent);
    _logPool->addCreationMulticast(type, ep, curTime, curevent++, myPe,
...
void TraceProjections::beginExecute(envelope *e)
{
  if(e==0) {
    ...
    execEp = (-1);
    _logPool->add(BEGIN_PROCESSING,ForChareMsg,_threadEP,TraceTimer(),
		  execEvent,myPe, 0, NULL, 0.0, TraceCpuTimer());
#if CMK_HAS_COUNTER_PAPI
    _logPool->addPapi(numPAPIEvents, papiEvents, papiValues);
#endif
    inEntry = 1;
...
void TraceProjections::endExecute(void)
{
  ...
  if(execEp == (-1)) {
    _logPool->add(END_PROCESSING, 0, _threadEP, TraceTimer(),
		  execEvent, myPe, 0, NULL, 0.0, cputime);
  } else {
...
{
  _logPool->add(BEGIN_IDLE, 0, 0, TraceTimer(curWallTime), 0, myPe);
}

void TraceProjections::endIdle(double curWallTime)
{
  _logPool->add(END_IDLE, 0, 0, TraceTimer(curWallTime), 0, myPe);
}

void TraceProjections::beginPack(void)
{
  _logPool->add(BEGIN_PACK, 0, 0, TraceTimer(), 0, myPe);
}

void TraceProjections::endPack(void)
{
  _logPool->add(END_PACK, 0, 0, TraceTimer(), 0, myPe);
}

void TraceProjections::beginUnpack(void)
{
  _logPool->add(BEGIN_UNPACK, 0, 0, TraceTimer(), 0, myPe);
}

void TraceProjections::endUnpack(void)
{
  _logPool->add(END_UNPACK, 0, 0, TraceTimer(), 0, myPe);
}
...
void TraceProjectionsBOC::shutdownAnalysis() {
  ...
  if (CkpvAccess(useOutlierAnalysis)) {
    thisProxy[myPe].startOutlierAnalysis();
  } else {
    thisProxy[myPe].startEndTimeAnalysis();
  }
}
