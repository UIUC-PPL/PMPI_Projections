...
MPI_Fint MPI_File_c2f (MPI_File file) {
	write_END_PROCESSING();
	long startTime = time_us();
	MPI_Fint ret = PMPI_File_c2f( file);
	write_EVENT_PAIR(MPI_File_c2f_event, startTime);
	write_BEGIN_PROCESSING();
	return ret;
}
...
MPI_Fint MPI_Info_c2f (MPI_Info info) {
	write_END_PROCESSING();
	long startTime = time_us();
	MPI_Fint ret = PMPI_Info_c2f( info);
	write_EVENT_PAIR(MPI_Info_c2f_event, startTime);
	write_BEGIN_PROCESSING();
	return ret;
}
...
MPI_Info MPI_Info_f2c (MPI_Fint info) {
	write_END_PROCESSING();
	long startTime = time_us();
	MPI_Info ret = PMPI_Info_f2c( info);
	write_EVENT_PAIR(MPI_Info_f2c_event, startTime);
	write_BEGIN_PROCESSING();
	return ret;
}
